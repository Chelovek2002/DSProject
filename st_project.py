...
with st.echo(code_location='below'):
    ...
    def sns_wage_corr(param):
        ...
        sns.pointplot(data=wage_df,
                      ...
                      scale=0.9
                      ).set(xlabel=dct['x_label'], ylabel='Salary (K)')
        ax.tick_params(axis='y', colors='#EFEFEF')
        plt.xticks(rotation=-dct['angle'])
    ...
    def bokeh_wage_corr(param):
        ...
        key = dict(zip(data['order'], range(len(data['order']))))
        if param == 'company_revenue':  # for some reason offset is slightly different here
            source = wage_df.assign(order=lambda x: x[param].map(key) - 0.5).sort_values(by='order')
        else:
            source = wage_df.assign(order=lambda x: x[param].map(key) + 0.5).sort_values(by='order')
    ...
    with st.container():
        anime_data = get_data_for_corr(anime_param)
        anime_order, anime_angle = anime_data['order'], anime_data['angle']
        max_y = max(wage_df[anime_param].value_counts())
        anime_x = alt.X(anime_param, sort=anime_order, type='ordinal',
                        axis=alt.Axis(labelAngle=anime_angle, labelColor='#EFEFEF', titleColor='#EFEFEF',
                                      labelFontSize=16, titleFontSize=20,
                                      title=anime_data['x_label']))
        ...
        def plot_animation(df, plot_opacity=1.0):
            anime_chart = alt.Chart(df).mark_bar(opacity=plot_opacity).encode(x=anime_x, y=anime_y).properties(
                width=600, height=300)
            return anime_chart


        slider_threshold = st.slider("Salary range", 0, 255, (100, 200))
        shadow_plot = plot_animation(wage_df, plot_opacity=0.3)
        slider_plot = plot_animation(
            wage_df[(wage_df.avg_salary > slider_threshold[0]) * (wage_df.avg_salary <= slider_threshold[1])])
        line_plot = st.altair_chart(slider_plot + shadow_plot)
        status_text = st.empty()
        status_text.text(f'Salary Range = ${slider_threshold[0]}K-${slider_threshold[1]}K')


        def start_animation():
            global line_plot, status_text, shadow_plot, slider_threshold
            ...
            for threshold in range(0, 255, 15):
                ...
                anime_chart = plot_animation(step_df)
                line_plot = line_plot.altair_chart(anime_chart + shadow_plot)
                time.sleep(0.7)
            ### END FROM
